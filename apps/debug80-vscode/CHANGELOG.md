...
- Added Glimmer as a first-class source type: `.glim` files in a
  ...
  checking, and a debug map that attributes block-body lines to the
  `.glim` source — breakpoints, stepping, and build errors all land in
  Glimmer source). Select explicitly with `"assembler": "glimmer"` or
  implicitly by the `.glim` extension.
- Added the `glim` language: syntax highlighting for `.glim` files with
  Z80 assembly highlighting embedded inside `begin`/`end` bodies,
  `begin`/`end` folding, and breakpoint support. The `glim` language is
  also listed in the debugger's `languages` contribution, matching its
  `breakpoints` entry.
- Updated AZM to `^0.3.4` (the 0.3 declaration model: `.routine`
  register contracts, `_name` owner-local labels, `@` as export syntax,
  and D8 symbol identity/visibility/sourceUnit metadata).
- Updated Glimmer to `^0.5.3` (generates AZM 0.3 syntax), so one
  deduped AZM serves both the AZM and Glimmer backends — the nested
  legacy AZM 0.2.17 install is gone.
- Completed the first-class Glimmer editor/project integration: explicit
  `"assembler": "glimmer"` configuration is accepted, Glimmer programs are
  discoverable and selectable without treating `part` files as targets, D8
  hover/definition providers run in `.glim` documents, and the Glimmer grammar
  has regression coverage plus distinct declaration/reactive/operator colours.
- Added a project-persisted **Case-sensitive symbols** checkbox. New and existing
  projects default to strict AZM symbol lookup; clearing it writes
  `azm.symbolCase: "insensitive"` for legacy source compatibility.
- Replaced modal-producing assembly launch failures with Problems diagnostics,
  revealed Debug80 Output, clean session termination, and an error-coloured
  project status line with a warning icon.

## 0.1.31

- Fixed TEC-1G reset entry handling, including Fn reset support and holding the
  reset key long enough for MON3 boot sampling.
- Reset MON3 presentation state on TEC-1G reset and added focused reset/status
  coverage.
- Synced `package-lock.json` for npm 10 clean installs.
- Updated the engineering manual for AZM register contract policy.

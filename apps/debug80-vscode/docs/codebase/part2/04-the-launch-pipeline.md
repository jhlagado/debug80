...
[← DAP and the Debug Session](03-dap-and-the-debug-session.md) | [Part II](index.md) | [Execution Control →](05-execution-control.md)
...
The successful path writes the HEX output, BIN output when present, a native D8 map, the default `.lst` listing, and any requested register-contract artifacts beside the HEX artifact. Register-contract reports are written as `.regcontracts.txt`; inferred interfaces are written as `.asmi`. When the build-only path enables **Contract Updates**, AZM can also return full replacement text for revised source files through a `register-contracts-annotations` artifact. `registerContractsPolicy` flows through unchanged when present, so one launch can ask AZM to enforce different register-contract modes for different source-file globs. Debug80 does not derive active source maps from the listing and does not write generated source-map cache files.

Those contract-update artifacts are not written by the backend. `assembleIfRequested()` now returns them to the extension host, and `buildCurrentProjectTarget()` decides whether to apply them as a `WorkspaceEdit`. `Ask` opens a modal prompt that can show a diff against the proposed text before applying it. `Auto` applies the edit immediately. `Never` discards the returned revisions. Run and restart launches do not request contract updates, so only the explicit Build path can rewrite source from AZM inference.
...
- The assembler backend is selected from the target configuration and source file extension. AZM handles `.asm`, `.inc`, and `.z80`; Glimmer handles `.glim`. Assembly is optional and conditional on the `assemble` flag.

...
Each symbol must carry either `address` or `value`. Symbols with a source line and an address become `SourceMapAnchor` entries during D8 import. Debug80 also preserves `identity`, `visibility`, and `sourceUnit` on the editor-facing symbol records that power F12, hover, and workspace-symbol lookup. Those extra fields let the definition provider distinguish exported declarations from source-private and owner-local declarations that share the same visible short name.

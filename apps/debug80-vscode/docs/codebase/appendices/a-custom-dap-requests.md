...
| `debug80/tec1gJoystick`    | `{ mask: number }`                                                                                | —             | Sets the active-low joystick overlay mask that is merged into matrix row 3 |
| `debug80/tec1gTms9918Active` | `{ enabled: boolean }`                                                                          | —             | Attaches or detaches the TEC-1G TMS9918/TMS9929 card at ports `0xBE/0xBF` |
| `debug80/tec1gTms9918VideoStandard` | `{ standard: 'pal' \| 'ntsc' }`                                                          | —             | Switches the TMS9918/TMS9929 frame cadence between PAL 50 Hz and NTSC 60 Hz |
| `debug80/tec1gReset`       | `{ fn?: boolean }`                                                                                | —             | Cold-resets the TEC-1G from the hardware reset vector while restoring MON-3 monitor RAM at `0x0800-0x0FFF`; `fn: true` latches Fn for the first post-reset keypad read |

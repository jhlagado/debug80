...
| Headless integration     | AZM build output running through the installed standalone runtime          |
...
- the CP/M 2.2 launch path publishes the host `.com` artifact, installs it into a private guest disk, preserves BIOS source mapping during the resulting terminal session, keeps the CP/M terminal webview aligned with full-screen editor control traffic, and preserves the bundled guest-tool contracts for Atom, Nucleus, and EDIT;
...
the root `npm run check` gate before narrowing down to `npm test -w debug80`, the AZM headless
integration, or tests in the standalone Runtime repository.

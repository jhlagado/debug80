...
This manual is updated against the codebase state through **2026-07-19**. These notes give maintainers a quick view of changes that affect several chapters:
...
- **Verification pipeline:** repository-wide `npm run check` now builds, type-checks, lints, format-checks, and tests AZM, Glimmer, Debug80 Runtime, the two headless integration workspaces, and the VS Code extension. GitHub Actions runs that root check plus package smoke checks for AZM, Glimmer, Debug80 Runtime, the packaged Glimmer headless integration workspace, and the packaged extension.

- **Assembler backends:** Debug80 ships four in-process build paths: Atom and AZM for assembly, Glimmer for `.glim`, and Nucleus for `.nu`. Atom is the default for assembly sources; a target that requires AZM selects it explicitly. The Debug80 launch path asks each backend for the artifacts needed by a debug session: Intel HEX for loading, native `.d8.json` for source mapping, and backend-specific companion files. Atom publishes HEX, BIN, D8, and listing artifacts transactionally; standalone Atom CLI and API callers can still request a smaller explicit output set.
...
- **Target discovery:** project targets are discovered across the workspace from runnable source files rather than from a fixed `src/` layout. Files named exactly `main.asm` or `main.z80`, plus `.glim` files that declare a top-level `program`, are offered as runnable targets. Other `.asm`, `.z80`, and `.glim` program files can still be added to `debug80.json` through the target-management commands.

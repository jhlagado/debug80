...
- **Assembler backends:** Debug80 now ships two in-process build paths that both emit native `.d8.json` maps: AZM for `.asm`, `.inc`, and `.z80`, plus Glimmer for `.glim`. The old listing-derived mapping path has been removed from active project behaviour. Active targets should be expected to build a HEX plus a native D8 source map. AZM-backed builds also write an asm80-style `.lst` listing beside the main artifacts unless the backend is told not to emit it.

...
The repository and build-boundary notes below reflect the September 2026 extraction. The remaining platform notes retain their earlier implementation checkpoints; this migration does not constitute a new hardware audit.

- **Repository layout:** Debug80 retains `apps/debug80-vscode`, `packages/azm`, and `integration/azm-headless` as workspaces. Atom, Nucleus, Runtime, and Tool Services are installed from pinned public Git revisions. Their source trees are maintained in standalone repositories.
...
- **Verification pipeline:** `npm run check` builds and verifies the three retained workspaces and checks assembler selection, the AZM import inventory, Atom-generated debugger fixtures, and CP/M byte equivalence. Standalone repositories run their own source and package proofs. Debug80 CI also checks the VSIX and runs extension-host tests.

- **Assembler backends:** Debug80 ships three in-process build paths: Atom and AZM for assembly, and Nucleus for `.nu`. Atom is the default for assembly sources; a target that requires AZM selects it explicitly. Launch builds produce Intel HEX for loading, native `.d8.json` for source mapping, and backend-specific companion files. Glimmer is no longer a shipped backend.
- **Editor grammars:** Debug80 contributes TextMate grammars for Z80 assembly and Nucleus, with breakpoint registration for those languages. Glimmer grammar and activation entries have been removed.
- **Target discovery:** files named `main.asm`, `main.z80`, or `main.nu` are runnable entry candidates anywhere outside excluded build and metadata directories. Other `.asm`, `.z80`, and `.nu` files can be added through target-management commands.
- **Target management:** the project header and command surface can now persist discovered sources as named targets, remove configured targets while keeping source files on disk, return a project to a zero-target state, rename targets, retarget program files, and adjust per-target `artifactBase`, `outputDir`, platform override, and assembler selection in `debug80.json`.
...
- **TEC-1G hex keypad hold model:** the TEC-1G keypad no longer behaves as a fixed one-shot pulse. `debug80/tec1gKey` now accepts press and release transitions, the runtime keeps the selected key latched until release while still enforcing a minimum pulse for short taps, and the shared headless session API exposes matching press/release helpers so Glimmer `held` bindings and MON-3 `scanKeys` polling see sustained keypad input.
...
- **AZM build diagnostics:** AZM failures carry source-root-resolved file paths, line/column information and source text when available into `debug80/assemblyFailed`. Debug80 reports the error through the Debug Console, the revealed Debug80 Output channel, VS Code Problems, a dedicated build-failure badge beside the Run button, and a build-status line that is separate from the CoolTerm hardware-send status. Explicit run and restart paths clear the last build failure before launch, and a successful build emits `debug80/assemblySucceeded` so the build-status warning is removed without disturbing hardware status text. The adapter completes and terminates a failed build session without returning the launch error that opens VS Code's launch-configuration modal. A nominal AZM success without a D8 map or without HEX data records is treated as an assembly failure.

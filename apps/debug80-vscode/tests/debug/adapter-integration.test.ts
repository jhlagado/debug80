...
describe('adapter integration', () => {
  ...
  it('reports assembly failure and terminates without rejecting the launch request', async () => {
    ...
    try {
      ...
      if (simpleKit === undefined) {
        throw new Error('Missing simple/default project kit');
      }
      fs.mkdirSync(path.join(root, 'src'), { recursive: true });
      fs.writeFileSync(
        path.join(root, 'src', 'main.asm'),
        '        ORG 0900h\nSTART:  JP MissingSymbol\n'
      );
      fs.writeFileSync(
        path.join(root, 'debug80.json'),
        `${JSON.stringify(
      ...
      await expect(terminatedEvent).resolves.toBeDefined();
    } finally {

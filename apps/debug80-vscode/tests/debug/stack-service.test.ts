...
describe('stack-service', () => {
  ...
  it('resolves the active bank source when expansion banks share an address', () => {
    ...
    expect(result).toEqual({ path: canonicalizeDebuggerSourcePath('/test/bank0.asm'), line: 9 });
  });
  ...
  it('uses the active bank source for the current stack frame', () => {
    ...
    expect(frames.stackFrames[0]?.source?.path).toBe(
      canonicalizeDebuggerSourcePath('/test/bank0.asm')
    );

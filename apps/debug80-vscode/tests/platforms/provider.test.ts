...
describe('platform providers', () => {
  ...
  it('registers TEC-1 commands through the platform registry', async () => {
    ...
    keyHandler?.({} as DebugProtocol.Response, { code: 0x41, pressed: false });
    expect(context.sessionState.tec1Runtime?.applyKey).toHaveBeenLastCalledWith(0x41, false);
  ...
  it('registers TEC-1G reset with hardware reset entry and MON-3 menu presentation', async () => {
    const provider = await resolvePlatformProvider({
      platform: 'tec1g',
      tec1g: { entry: 0x0000 },
    });
    ...
    context.sessionState.tec1gRuntime = {
      resetState: vi.fn(),
      holdKeyForReset: vi.fn(),
    } as never;
    context.sessionState.loadedProgram = program as never;
    context.sessionState.loadedEntry = 0x4000;
    ...
    expect(memory[0x0886]).toBe(0x00);
    expect(memory[0x0887]).toBe(0x40);
    expect(memory[0x0888]).toBe(0x08);
    expect(memory[0x0889]).toBe(0x55);
    expect(memory[0x4000]).toBe(0x3e);
    expect(context.sessionState.tec1gRuntime?.resetState).toHaveBeenCalledTimes(1);
  });

  it('registers TEC-1G Fn reset as a reset-time key latch', async () => {
    const provider = await resolvePlatformProvider({
      platform: 'tec1g',
      tec1g: { entry: 0x0000 },
    });
    ...
    const resetHandler = registry.getHandler('debug80/tec1gReset');
    expect(resetHandler).toBeDefined();
    resetHandler?.({} as DebugProtocol.Response, { fn: true });

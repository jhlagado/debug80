...
describe('tec1g ui-panel-messages', () => {
  ...
  it('forwards Fn reset state before reasserting matrix mode', async () => {
    const { ctx } = createContext();
    const customRequest = vi.fn().mockResolvedValue(undefined);
    ctx.getSession = () => ({ type: 'z80', customRequest });

    await handleTec1gMessage({ type: 'reset', fn: true, matrixModeAfterReset: true }, ctx);

    expect(customRequest).toHaveBeenNthCalledWith(1, 'debug80/tec1gReset', { fn: true });
    expect(customRequest).toHaveBeenNthCalledWith(2, 'debug80/tec1gMatrixMode', { enabled: true });
  });

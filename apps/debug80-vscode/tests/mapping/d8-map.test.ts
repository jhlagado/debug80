...
describe('d8-map', () => {
  ...
  it('accepts AZM 0.3 declaration identity and visibility metadata', () => {
    const payload = {
      format: 'd8-debug-map',
      version: 1,
      arch: 'z80',
      ...
      files: {
        'src/main.asm': {
          symbols: [
            {
              name: 'Draw._done',
              identity: 'src/main.asm:8:1:label:Draw._done',
              address: 0x4008,
              line: 8,
              scope: 'local',
              visibility: 'local',
    ...
    const { map, error } = parseD8DebugMap(JSON.stringify(payload));

    expect(error).toBeUndefined();
    expect(map?.files['src/main.asm']?.symbols?.[0]).toMatchObject({
      identity: 'src/main.asm:8:1:label:Draw._done',
      visibility: 'local',
      sourceUnit: 'src/main.asm',
    });
  ...
  it('rejects invalid map shapes and field types', () => {
    const cases: Array<[string, unknown]> = [
      ...
      [
        'Symbol identity must be a string.',
        {
          format: 'd8-debug-map',
          version: 1,
          arch: 'z80',
          addressWidth: 16,
          endianness: 'little',
          files: { 'src/main.asm': { symbols: [{ name: 'X', address: 0, identity: 1 }] } },
        },
      ],
      [
        'Symbol visibility is invalid.',
        {
          format: 'd8-debug-map',
          version: 1,
          arch: 'z80',
          addressWidth: 16,
          endianness: 'little',
          files: { 'src/main.asm': { symbols: [{ name: 'X', address: 0, visibility: 'bad' }] } },
        },
      ],
      [
        'Symbol sourceUnit must be a string.',
        {
          format: 'd8-debug-map',
          version: 1,
          arch: 'z80',
          addressWidth: 16,
          endianness: 'little',
          files: { 'src/main.asm': { symbols: [{ name: 'X', address: 0, sourceUnit: 1 }] } },
        },
      ],
    ];

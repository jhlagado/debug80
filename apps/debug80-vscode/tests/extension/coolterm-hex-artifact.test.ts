...
type CoolTermHexFixture = {
  cleanup(): void;
  makeProject(config: Debug80ProjectConfig): string;
  writeHexArtifact(root: string, relativePath: string): string;
};

function createCoolTermHexFixture(): CoolTermHexFixture {
...
describe('resolveCoolTermHexArtifact', () => {
  const fixture = createCoolTermHexFixture();
  ...
  it('resolves an explicit target hex path', () => {
    const root = fixture.makeProject({
      targets: {
        app: { hex: 'build/app.hex' },
      },
    });
    const hexPath = fixture.writeHexArtifact(root, 'build/app.hex');
  ...
  it('infers outputDir/artifactBase when target hex is omitted', () => {
    ...
    const hexPath = fixture.writeHexArtifact(root, 'build/monitor.hex');

...
vi.mock('../../src/extension/project-config', () => ({
  ...
  resolveProjectAzmSymbolCase: vi.fn(() => 'strict'),
...
describe('PlatformViewProvider', () => {
  ...
  it('persists AZM symbol case changes from the project checkbox', async () => {
    workspaceFolders = [{ name: 'demo', uri: { fsPath: '/workspace/demo' } }];
    const provider = new PlatformViewProvider(extensionRoot);
    const webviewView = createWebviewView();
    provider.resolveWebviewView(
    ...
    const handler = (webviewView.webview.onDidReceiveMessage as ReturnType<typeof vi.fn>).mock
      .calls[0]?.[0] as (msg: { type: string; symbolCase: string }) => Promise<void>;

    await handler({ type: 'setAzmSymbolCase', symbolCase: 'insensitive' });

    expect(updateProjectAzmSymbolCase).toHaveBeenCalledWith(
      '/workspace/demo/debug80.json',
      'insensitive'
    );
  });
  ...
  it('posts full Tec1g display fields on every runtime update', async () => {
    ...
    const firstPayload: Tec1gUpdatePayload = {
      digits: [0, 0, 0, 0, 0, 0],
      matrix: [0, 0, 0, 0, 0, 0, 0, 0],
    ...
    (webviewView.webview.postMessage as ReturnType<typeof vi.fn>).mockClear();
    provider.updateTec1g(firstPayload);
    provider.updateTec1g({
      ...firstPayload,
      matrixGreen: [0x11, 0, 0, 0, 0, 0, 0, 0],
    });

    const updateCalls = getPostMessageCalls(webviewView).filter(([msg]) => msg.type === 'update');

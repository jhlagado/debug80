...
describe('TEC-1G SYS_CTRL status lamps', () => {
  ...
  it('passes the latched Fn state to the RESET override before clearing it', () => {
    const resetMessages: unknown[] = [];
    document.body.innerHTML = '';
    const keypadEl = document.createElement('div');
    document.body.appendChild(keypadEl);
    ...
    createTecKeypad({ postMessage: (message) => resetMessages.push(message) }, keypadEl, {
      onReset: (state) => resetMessages.push({ type: 'reset', fn: state.fn }),
    });

    const fnButton = Array.from(keypadEl.querySelectorAll<HTMLElement>('.keycap')).find(
      (button) => button.textContent === 'FN'
    );

    fnButton?.click();
    keypadEl.querySelector<HTMLElement>('.keycap-reset')?.click();

    expect(resetMessages).toEqual([{ type: 'reset', fn: true }]);
  });

  it('does not set Fn reset state when Fn is not latched', () => {
    const resetMessages: unknown[] = [];
    document.body.innerHTML = '';
    const keypadEl = document.createElement('div');
    document.body.appendChild(keypadEl);
    ...
    expect(resetMessages).toEqual([{ type: 'reset', fn: false }]);
  });

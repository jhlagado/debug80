...
describe('package.json language contracts', () => {
  ...
  it('launch schema exposes strict and insensitive AZM symbol lookup', () => {
    ...
    const azm = debuggerContribution?.configurationAttributes?.launch?.properties?.azm;
    const symbolCase = azm?.properties?.symbolCase;

    expect(symbolCase?.default).toBe('strict');
  ...
  it('Z80 assembly grammar highlights AZM 0.3 register-contract directives', () => {
    const serializedGrammar = JSON.stringify(loadZ80AsmGrammar());
    expect(serializedGrammar).toContain('#azm-register-contracts');
    expect(serializedGrammar).toContain('\\.routine');
    expect(serializedGrammar).toContain('maybe-out');
    expect(serializedGrammar).toContain('\\.contracts');
    expect(serializedGrammar).toContain('\\.rcignore');
    expect(serializedGrammar).toContain('\\.expectout');
    expect(serializedGrammar).not.toContain('@(?:routine|proc|extern)');
  });

  it('Z80 assembly grammar treats retired bang comments as ordinary comments', () => {
    const grammar = loadZ80AsmGrammar() as {
      repository?: Record<
        string,
        {
    ...
    expect(comments.some((pattern) => pattern.begin === ';!')).toBe(false);
    expect(comments.find((pattern) => pattern.begin === ';')?.name).toBe(
  ...
  it('Z80 assembly grammar captures AZM op invocations separately from symbols', () => {
    ...
    const invocationPattern = getGrammarPattern('azm-op-invocations', 'meta.op.invocation.z80-asm');
  ...
  it('Z80 assembly grammar distinguishes AZM exports and owner-local labels', () => {
    expect(getFirstMatchingGrammarScope('labels', '@DrawTile:')).toBe(
      'entity.name.label.exported.z80-asm'
    );
    expect(getFirstMatchingGrammarScope('labels', '_done:')).toBe(
      'entity.name.label.local.z80-asm'
    );
    expect(getFirstMatchingGrammarScope('labels', '@Draw_Tile:')).toBe(
      'entity.name.label.exported.z80-asm'
    );
    expect(getFirstMatchingGrammarScope('labels', '@.entry:')).toBe(
      'entity.name.label.exported.z80-asm'
    );
    expect(getFirstMatchingGrammarScope('labels', '@$entry:')).toBe(
      'entity.name.label.exported.z80-asm'
    ...
    expect(getFirstMatchingGrammarScope('labels', '_done_now:')).toBe(
      'entity.name.label.local.z80-asm'
    );
  });

  it('Z80 assembly grammar accepts comments after contract policy directives', () => {
    expect(
      getFirstGrammarCaptureScope(
        'azm-register-contracts',
        '.contracts audit ; retained monitor source',
        '.contracts'
      )
    ).toBe('keyword.control.contract.z80-asm');
    expect(
      getFirstGrammarCaptureScope(
        'azm-register-contracts',
        '.rcignore missing_contract "legacy entry" ; migration debt',
        '.rcignore'
      )
    ).toBe('keyword.control.contract.z80-asm');
  });

...
const PROJECT_CONFIG_CANDIDATES = ['debug80.json', path.join('.vscode', 'debug80.json')];
...
export function resolveProjectAzmSymbolCase(config: ProjectConfig | undefined): AzmSymbolCaseMode {
  return config?.azm?.symbolCase === 'insensitive' ? 'insensitive' : 'strict';
}

export function updateProjectAzmSymbolCase(
  ...
  return writeProjectConfig(projectConfigPath, {
    ...config,
    azm: {
      ...(config.azm ?? {}),
      symbolCase,

...
const TEC1G_MON3_CEL = 0x0886;
const TEC1G_MON3_MCB = 0x0888;
const TEC1G_MON3_DEFAULT_USER_ADDR = 0x4000;
const TEC1G_MON3_DEFAULT_MCB = 0x08;
const TEC1G_HARDWARE_RESET_ENTRY = 0x0000;
const TEC1G_FN_KEY_CODE = 0x02;
 ...
 *
 */
function isFnReset(args: unknown): boolean {
  return typeof args === 'object' && args !== null && (args as { fn?: unknown }).fn === true;
}

/**
 * Normalizes the MON3 user-facing reset state after preserving monitor RAM.
 *
 * MON3 uses MCB bit 3 to remember LCD/menu versus segment address/data mode, and its warm
 * boot path intentionally preserves that bit. Debug80's reset button should behave like a
 * fresh monitor reset from the user's point of view, so it keeps the RAM signature needed
 * for MON3 warm boot detection while restoring the default menu presentation state.
 */
function resetMon3PresentationState(memory: Uint8Array | undefined): void {
  if (!memory || memory.length <= TEC1G_MON3_MCB) {
    return;
  }
  memory[TEC1G_MON3_CEL] = TEC1G_MON3_DEFAULT_USER_ADDR & 0xff;
  memory[TEC1G_MON3_CEL + 1] = (TEC1G_MON3_DEFAULT_USER_ADDR >> 8) & 0xff;
  memory[TEC1G_MON3_MCB] = TEC1G_MON3_DEFAULT_MCB;
}
    ...
    commands: {
      ...
      'debug80/tec1gReset': (response, args) =>
        sendPlatformResponse(
          ...
          context,
          () => {
            resetMon3PresentationState(context.sessionState.runtime?.hardware.memory);
            if (isFnReset(args)) {
              context.sessionState.tec1gRuntime?.holdKeyForReset(TEC1G_FN_KEY_CODE);
            }

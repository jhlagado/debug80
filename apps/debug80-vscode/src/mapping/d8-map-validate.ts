...
const SYMBOL_VISIBILITY_SET = new Set<D8SymbolVisibility>(['exported', 'source', 'local']);
...
export function validateD8DebugMap(value: unknown): string | undefined {
  ...
  const validateSymbol = (symbol: unknown): string | undefined => {
    ...
    if (symbol.identity !== undefined && typeof symbol.identity !== 'string') {
      return 'Symbol identity must be a string.';
    }
    if (
      symbol.visibility !== undefined &&
      !SYMBOL_VISIBILITY_SET.has(symbol.visibility as D8SymbolVisibility)
    ) {
      return 'Symbol visibility is invalid.';
    }
    if (symbol.sourceUnit !== undefined && typeof symbol.sourceUnit !== 'string') {
      return 'Symbol sourceUnit must be a string.';
    }
    return undefined;
  };

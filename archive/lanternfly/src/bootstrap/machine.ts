...
/**
 * Seek the code output, low byte then high byte. Writing the high byte
 * commits the seek.
 *
 * **The argument is an offset into the output stream, not a target address.**
 * The stream begins at offset zero whatever origin the compiler is emitting
 * for, so a compiler tracking addresses converts before seeking. A flat image
 * based at zero makes the two coincide, which is exactly why the distinction
 * has to be stated rather than discovered later.
 *
 * Lanternfly is single-pass, so a forward jump is emitted with a placeholder
 * and back-patched when its target is reached. Back-patching needs a writable
 * output, and the alternative — buffering the whole image in memory and
 * flushing at the end — does not fit: at the upper end of the code estimate it
 * leaves negative headroom, because the buffer is as large as the compiler.
 *
 * A disk-based single-pass compiler has always had this. It is one service
 * rather than an image-sized buffer.
 */
export const PORT_SEEK_LOW = 0x06;
export const PORT_SEEK_HIGH = 0x07;
...
export class BootstrapMachine {
  ...
  /** Undefined while appending; an index while a patch is being written. */
  #codeCursor: number | undefined = undefined;
  #seekPending: number | undefined = undefined;
  /** Writes past the emitted end, which are compiler bugs rather than patches. */
  #codeFaults = 0;
  ...
  #write(port: number, value: number): void {
    switch (port) {
      case PORT_CODE:
        // Appends at the end, or overwrites at the cursor when one is set.
        // A patch is a seek, a write, and a seek back.
        if (this.#codeCursor === undefined) {
          this.#code.push(value);
          return;
        }
        if (this.#codeCursor >= this.#code.length) {
          // A patch that ran off the end of what was emitted. Growing the
          // output would hide the fault.
          this.#codeFaults += 1;
          return;
        }
        this.#code[this.#codeCursor] = value;
        this.#codeCursor += 1;
        // Patching to exactly the end returns to appending, so a two-byte
        // operand at the very end does not then need a seek to resume.
        if (this.#codeCursor === this.#code.length) this.#codeCursor = undefined;
        return;
      case PORT_SEEK_LOW:
        this.#seekPending = value;
        return;
      case PORT_SEEK_HIGH: {
        const offset = ((value << 8) | (this.#seekPending ?? 0)) & 0xffff;
        this.#seekPending = undefined;

        if (offset === this.#code.length) {
          // Exactly the end resumes appending, which is how a compiler
          // returns to emitting after paying off a patch.
          this.#codeCursor = undefined;
          return;
        }
        if (offset > this.#code.length) {
          // Past the end is a compiler bug. Treating it as append would leave
          // a hole in the image and hide the fault.
          this.#codeFaults += 1;
          this.#codeCursor = undefined;
          return;
        }
        this.#codeCursor = offset;
        return;
      }
  ...
  /** Writes that fell outside the emitted output. Nonzero means a bug. */
  codeFaults(): number {
    return this.#codeFaults;
  }

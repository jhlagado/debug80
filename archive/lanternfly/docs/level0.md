...
**Names compare exactly.** Two identifiers are the same name when their
case-folded spellings are equal, and a conforming compiler implements that.

**Candlemoth does not**, and the difference is a bootstrap-compiler limitation
rather than a level-0 rule. It stores two sixteen-bit hashes and no spelling,
which saves 5,568 bytes of a small machine's memory and makes two identifiers
whose hashes both agree indistinguishable — a silent miscompile it cannot
detect. `test/names.test.ts` verifies that Candlemoth's own identifiers do not collide, which is the one input it must handle to exist at all. The seed
compares exactly, so the two implementations differ here on purpose and the
fixpoint compares Candlemoth against itself rather than against the seed.

**No strings.** Identifiers are interned and thereafter carried as an index. They are not offsets into the
...
- `forward sub`, for mutual recursion in the ordinary way. It does **not**
  ...
  inside a strongly-connected component of the call graph, and a declaration
  carries a name and a signature while the call edges live in the bodies. A
  revision of this document once appended a claim that `forward` settles it
  after all, contradicting the sentence it was appended to. The edges are
  collected as the bodies are read and the components computed once the source
  ends.
...
What remains is I/O, and the bootstrap profile supplies it as **six
intrinsics** rather than a binding mechanism. Each lowers to one or two
instructions and needs no symbol table, no address binding and no service
registry:
...
| `writeCodeByte(b as u8)` | `OUT (0x01),A` | write at the output cursor |
| `seekCode(offset as u16)` | `OUT (0x06),A` / `OUT (0x07),A` | move the output cursor |
| `writeDiagnostic(b as u8)` | `OUT (0x03),A` | append to the diagnostic stream |
| `setExitStatus(b as u8)` | `OUT (0x02),A` | record the outcome |

**`seekCode` takes an offset into the output stream, not a target address.**
The stream starts at zero whatever origin the code is emitted for, so a
compiler tracking addresses subtracts the image origin before seeking. A flat
image based at zero makes the two coincide, which is why the distinction has to
be written down rather than discovered.

It exists because Lanternfly is single-pass: a forward jump emits a
placeholder and is back-patched when its target is reached, and that needs
output already written to be reachable. Seeking exactly to the end resumes
appending; seeking past it is a fault, because a patch beyond what exists is a
compiler bug and appending would leave a hole in the image.

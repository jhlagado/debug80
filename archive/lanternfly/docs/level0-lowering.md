...
allocation state has to survive a second reading of the source.
...
**A forward target is back-patched.** Lanternfly is single-pass. A jump whose
target is not yet known emits a placeholder and records where the operand
sits; `placeLabel` writes the target in when it is reached, through the
`seekCode` service, whose argument is an offset into the output stream rather
than a target address.

Declaration before use and `forward` are consistent with single-pass
compilation and are what make it workable. They do **not** prove it: a
multi-pass compiler can enforce declaration before use perfectly well, and
language semantics do not mandate a compiler architecture. Lanternfly is
single-pass by decision.

Against the three-pass label table, **allocated**: 896 bytes of label pool and
patch lists against 1,024, a saving of 128. An earlier revision of this
paragraph claimed 54 against 1,024 by comparing live entries with an allocated
table and omitting call patching entirely.
...
**Save-around-call is settled by the call graph, and `forward` does not supply
one.** A declaration carries a name and a signature; the call edges live in
the bodies. `bootstrap-plan.md` records that an earlier draft proposed
requiring every routine to be forward-declared so the graph would be known up
front, and why that fails — and a later revision of this document made the
same mistake again.

In one source pass the edges are collected as the bodies are read, the
strongly-connected components are computed once the source ends, and a save
and a restore stub are emitted per routine that needs them. Every call site is
a fixed three-byte `CALL` whose operand is patched to the routine or to its
stub, which is what keeps the call size independent of an answer that arrives
later.

**None of that is implemented.** The compiler collects the edges and computes
nothing, so a recursive routine miscompiles today.

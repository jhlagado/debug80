...
the six profile intrinsics take arguments a parameterless language cannot
...
anything self-hosts, and the compiler's core machinery — back-patching a
forward jump through a streaming output, the fixpoint — has never run against
a real program.

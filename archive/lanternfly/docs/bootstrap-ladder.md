...
the two differ; that document's machine, budget, single-pass architecture and
...
proves back-patching a forward jump through a streaming output, and the
compiler's own machinery end to end — on 29 productions rather than 45, with
no parameters, no locals and no static frames. **If the architecture is wrong, it is wrong cheaply.**
...
The plan's machine, memory model, budget accounting, single-pass architecture

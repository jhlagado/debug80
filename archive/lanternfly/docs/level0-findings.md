...
### 19. RETRACTED — the streaming output did not forbid back-patching

This finding said the output could not be seeked, so a forward branch could not
be filled in once its target was reached, and that three passes plus a
program-sized label table were the price. It called that "the strongest
argument yet for the three-pass design".

**The premise was a property of the harness, not of the machine.** Phase 0 made
`writeCodeByte` an append-only port. Nothing about a Z80, or about an SD card,
requires that. The machine now has a seek service on ports `$06` and `$07`, and
a forward jump is back-patched the way Pascal has always done it.

The finding also missed what the language was saying. **Declaration before use
and `forward` are single-pass features** — a compiler that read the whole
program before emitting anything would need neither, and Lanternfly has both.
The design had stated its shape in the specification and this ledger argued
against it from a port.

What the correction costs and saves:

| | Then | Now |
| --- | --- | --- |
| Label bookkeeping | 512-entry table, 1,024 bytes | 32-label pool and 64-entry patch list |
| Peak live entries | every label in the program, 502 | unresolved only, 27 |
| Source walked | three times | once |
| Host services | five | six |

The one thing three passes bought was an address-agreement check between
layout and emission. A single-pass compiler has nothing to compare against, so
its check is that the patch list is empty at the end — one entry left over
would ship as a jump to address zero. The fixpoint already compares two whole
compilations, which is stronger than either.

**The pattern worth keeping.** Two of this ledger's retractions — findings 3
and 5 — came from claiming a language limitation without reading the
neighbouring specification section. This one is the same failure at a larger
scale: an architecture argued from a harness detail, while the language's own
features said otherwise.

### 34. `forward` does not supply the call graph, and I claimed twice that it did

Removing the three-pass architecture, I wrote that `forward` settles
save-around-call: that in a single-pass language with declaration before use a
cycle must pass through a forward declaration, so a call that could recurse is
a self-call or a call to a forward-declared routine, known at the call site.

**It is wrong.** A declaration carries a name and a signature. The call edges
live in the bodies, and strongly-connected components are computed from edges.
Knowing a callee was forward-declared says nothing about whether anything calls
back.

**`bootstrap-plan.md` already said so**, in a paragraph headed *Save-around-call
is settled by analysis, not by declarations*, which records that an earlier
draft proposed exactly this and why it fails. So the mistake was made, written
down as a mistake, and then made again by someone who had read the document
that records it.

Two smaller errors travelled with it.

**Declaration before use and `forward` do not prove single-pass compilation.**
They are consistent with it and are what make it workable; a multi-pass
compiler can enforce declaration before use perfectly well. Language semantics
do not mandate a compiler architecture. Lanternfly is single-pass by decision,
not by derivation, and stating it the other way dressed a ruling as a proof.

**Removing the passes without adding call patching broke forward calls.** A
call to a `forward sub` emitted `symbolAddress[slot]` before the body had set
it — which is zero, a call into the reset vector. The three-pass draft did not
have the bug because its layout pass filled every address before emission read
one. `emitCall` and `placeRoutine` now patch call operands the way jump
operands are patched.

The call graph is now collected as a bit matrix — 2,048 bytes, fixed, against
over three kilobytes for an edge list that grows with the call count. **The
components are still not computed and no save or restore stub is emitted, so a
recursive routine miscompiles today.**

### 35. The single-pass claim was measured against the wrong figure

"54 bytes of patch list against 1,024 bytes of label table" appeared in four
places. It compared **live** patch entries with an **allocated** table, and it
omitted call patching entirely.

Allocated against allocated:

| | Bytes |
| --- | --- |
| Three-pass label table, `u16[512]` | 1,024 |
| Label pool and label patch lists | 384 |
| Call patch lists | 512 |
| **Single-pass total** | **896** |

A saving of 128 bytes, not 970. The call graph — 2,048 for the matrix plus
1,280 for the routine index — is needed by both architectures and was unbuilt
in both, so it belongs to neither side of the comparison.

The habit worth naming: **a live-versus-allocated comparison flatters whichever
side is measured live**, and the compiler pays for what is allocated.
...
leaving through one. Against that the writable arrays are comfortable; `docs/bootstrap-plan.md`
carries the generated total, which is the one place it appears.

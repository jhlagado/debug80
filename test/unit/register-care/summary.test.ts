...
import { applyRoutineContract, inferRoutineSummary } from '../../../src/register-care/summary.js';
...
function instruction(text: string, line: number): RegisterCareInstruction {
  ...
  if (!parsed?.instruction)
    throw new Error(`parse failed: ${text}: ${parsed?.error ?? JSON.stringify(parsed?.diagnostics ?? [])}`);
  return {

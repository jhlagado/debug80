...
```typescript
function applyMatrixBrightness(dots: HTMLElement[], r: number[], g: number[], b: number[]): void {
  dots.forEach((dot, idx) => {
    const rv = r[idx] ?? 0;
    const gv = g[idx] ?? 0;
    const bv = b[idx] ?? 0;
    dot.style.backgroundColor = `rgb(${rv}, ${gv}, ${bv})`;
  });
}
```

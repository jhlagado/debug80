...
This manual is updated against the codebase state through **2026-06-28**. These notes give maintainers a quick view of changes that affect several chapters:
...
- **TEC-1G bank-aware source mapping:** when multiple expansion-ROM banks share the same visible `0x8000-0xBFFF` window, Debug80 now tags ROM-artifact D8 maps with a physical-bank address space. Breakpoints, source-map status, stack frames, nearest-symbol lookup, run-to-here targets, and one-shot breakpoint-skip handling all match the active bank first and only fall back to address-only segments when no bank-specific mapping exists.

...
for AZM source-row construction and target-per-source bindings. The tests still
cover remembered/default target row labels, bound source rows, unbound source
rows that can be applied to an existing target, and omission of unbound source
rows when no target can be bound.

This is intentionally test-only cleanup. It keeps target QuickPick policy
coverage intact while moving repeated option and binding setup out of the
behavior assertions.
...
temporary workspace files. The tests still cover target entry-source naming
conventions, recursive project-relative discovery, build folder exclusion, and
root-level target discovery without requiring a `src` folder.

This is intentionally test-only cleanup. It keeps target discovery coverage
intact while moving repeated temp-file path construction and default source
contents out of the behavior assertions.
...
helper for tests that need a temporary `debug80.json` manifest on disk. The
tests still cover target source updates, project platform resolution, stop-on-entry
resolution, initialized project detection, manifest validation, profile metadata
upgrade, and stale assembler id clearing.

This is intentionally test-only cleanup. It keeps project config helper
coverage intact while moving repeated temp manifest path construction and JSON
write scaffolding out of the behavior assertions.

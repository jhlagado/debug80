...
Assembler-specific module-system behavior is isolated behind backend adapters:

- `@jhlagado/azm` is ESM and is linked in-process by `src/debug/launch/azm-backend.ts` using
  dynamic `import()`.
...
1. **Bundled runtime dependencies**
   - `@jhlagado/azm` must remain in `dependencies`, not `devDependencies`.
   - `npm run package` must include AZM in the VSIX.
...
- confirming AZM can still be loaded cleanly;

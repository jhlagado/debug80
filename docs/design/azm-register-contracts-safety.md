...
- register contracts analysis over ordinary ASM80-style AZM source
...
Register contracts inference depends on knowing where a routine starts and ends. The
...
span are internal waypoints for register contracts purposes, even though they remain
...
- keep data labels outside routine-contract generation; inline data labels need
  an explicit source convention before register contracts can analyze around them
  safely
...
normal register contracts unit.
...
register contracts diagnostic at that call site. It should not globally change the
...
Internally, `F` does not exist as a true register contracts unit. Source-facing
...
The full register contracts system needs to model the stack, not just registers. In
...
A full register contracts system is best built in layers:
...
external register contracts without comment leaders.
...
12. emitting inferred callee contracts as AZMDoc comments or external register contract data
...
4. `AF` should decompose into `A` plus individual flags for compatibility; `F`
   is not an internal register contracts unit and should not appear in generated

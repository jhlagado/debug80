...
**`pr151_zero_operand_head_diag_matrix`** — port oracle zero-operand known-head diagnostic matrix (`test/fixtures/pr151_zero_operand_head_diag_matrix.asm`) as a compile-time integration test under `test/integration/`. Same lane as pr146–pr150: assert explicit arity messages and no `Unsupported instruction:` cascade.

Parallel sub-agent lanes within that single PR (optional): remaining **`pr129`–`pr143`** small ISA arity/invalid fixtures (one integration file per matrix where fixtures already exist under `test/fixtures/`).

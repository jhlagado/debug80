...
**Status:** In progress (Phase 3). Primary opcode table + register/ALU decoder extracted to `decode-primary.ts`. CB, ED, DD, FD, and DDCB prefix handlers extracted to `decode-cb.ts`, `decode-ed.ts`, `decode-dd.ts`, `decode-fd.ts`, and `decode-ddcb.ts`. DD and ED opcode tables now live in `decode-dd.ts` and `decode-ed.ts` (removed from `decode.ts`). Common decode helpers are now centralized in `decode-helpers.ts`, and `decode.ts` delegates helper construction.

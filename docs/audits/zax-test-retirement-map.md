...
- Tests for `func`, generated frames, local `var`, typed arguments, typed calls,
  structured `if`/`while`/`repeat`/`select`, `:=`, typed storage lowering, and
  implicit typed effective-address lowering are ZAX compatibility or retirement
  candidates.
...
| `test/pr163_import_extern_base_relative_call.test.ts` | imported extern base calls | ZAX compatibility | Preserve temporarily; module/link behavior may be retired or rewritten. |
...
1. Add an explicit compatibility test script for preserved `.zax` behavior so
   normal AZM alpha guardrails can stop depending on high-level ZAX constructs.

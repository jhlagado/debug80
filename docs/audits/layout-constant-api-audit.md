...
| Typed data storage lowering | `src/frontend/parseData.ts`, `src/lowering/programLoweringData.ts`, `test/pr1049_record_named_init_data_lowering.test.ts` | Parses typed `data` declarations, positional/named record initializers, strings, inferred array lengths, and lowers them to bytes. `.azm` mode rejects typed data blocks. | Retire from AZM-native. Keep tests only as inherited ZAX retirement evidence or as fixtures for raw `.db`/`.dw` migration examples. |

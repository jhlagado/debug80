...
| `offsetof` | `src/frontend/parseImm.ts`, `src/semantics/env.ts`, `src/semantics/layout.ts`, `src/lowering/loweredAsmStreamRecording.ts`, `src/lowering/opSubstitution.ts` | Immediate parser creates `ImmOffsetof` with a field/index path. Semantic evaluation walks records, unions, and arrays using exact sizes and compile-time index evaluation. | Keep and extend for arrays/nested paths. Lock `offsetof(Sprite, field)`, nested fields, and `offsetof(Sprite[16], [2].field)` before further implementation. |
...
- `test/semantics/layout_edge_cases.test.ts`: direct semantic tests for empty records/unions, nested record sizes, union fields with array members, arrays of unions, nested `offsetof`, array-index `offsetof`, and union offset rules.
- `test/fixtures/pr257_offsetof_valid.zax`: compile fixture proving nested record/array/union `offsetof` forms such as `offsetof(Scene, sprites[Idx].color)` and `offsetof(Node, payload.asWord)`.
...
2. There is no focused compile test for `offsetof(Sprite[16], [2].field)` as the public array-path spelling described in the AZM design.

...
|------|---------|------------------|--------------|
| Type declarations | `src/frontend/parseTypes.ts`, `src/frontend/parseRecordFieldDecl.ts`, `src/frontend/ast.ts`, `src/zaxImportVisibility.ts` | Parses `type Name TypeExpr`, record-style `type Name ... end`, and `union Name ... end`; stores `TypeDeclNode` and `UnionDeclNode` in the compile environment. ZAX import visibility is still present for the retirement lane. | Keep as layout metadata. AZM should retain type and union declarations when they only describe byte layout. |
...
| Explicit layout-cast effective addresses | `src/frontend/parseOperands.ts`, `src/frontend/ast.ts`, `src/lowering/eaResolution.ts`, `src/lowering/ldFormSelection.ts`, `src/lowering/valueMaterializationRuntimeEa.ts` | Parses `<TypeExpr>base[index].field` as `EaReinterpret` plus `EaField`/`EaIndex`. Current lowering can resolve constants, stack bases, and some runtime index paths. `.azm` native mode accepts only constant layout-cast address expressions and rejects hidden runtime lowering. | Keep only the constant-address query form. Runtime register indexes should remain rejected for the layout-constant feature. |
| Typed assignment | `src/frontend/parseAssignmentInstruction.ts`, `src/frontend/azmNativeRemovals.ts`, `src/lowering/asmLoweringAssign.ts`, `src/lowering/asmInstructionLowering.ts` | Parses and lowers `:=` typed assignments through inherited ZAX machinery; `.azm` mode rejects it with AZM700. | Retire from AZM-native source. It hides loads/stores and is outside the layout-constant subset. |
| Hidden typed memory lowering | `src/lowering/eaResolution.ts`, `src/lowering/addressingPipelines.ts`, `src/lowering/valueMaterializationBase.ts`, `src/lowering/valueMaterializationRuntimeEa.ts`, `src/lowering/asmLoweringStep.ts`, `src/lowering/valueMaterializationIndexing.ts` | Resolves typed fields/indexes and may emit address-materialization or indexed access sequences, including exact-scale runtime indexing support. | Retire from AZM-native unless reintroduced through explicit AST ops. The first AZM layout slice should fold constants only and emit no hidden address-calculation code. |
| Typed data storage lowering | Removed in the AZM removal pass. | Typed `data` declarations, positional/named record initializers, strings, inferred array lengths, and data-block lowering are no longer live AZM code. | Use labels plus `.db`/`.dw`/`.ds` and layout constants. |
| Typed var/global storage | `src/frontend/parseModuleCommon.ts`, `src/semantics/storageView.ts`, `src/lowering/programLoweringTraversal.ts`, `src/lowering/functionFrameSetup.ts` | Uses type expressions to size module/global/local storage and to attach typed effective-address metadata to symbols. | Retire from AZM-native. AZM should prefer labels plus `.db`/`.dw`/`.ds sizeof(...)` constants. |
...
4. The explicit layout-cast syntax is currently tied to `EaReinterpret`, while `.azm` native removal logic must distinguish constant layout casts to keep from runtime typed access to reject.
...
7. Typed var/global storage and typed assignment still share layout helpers with the desired constant feature. Follow-up removal work needs tests that prove the helpers survive after those high-level surfaces are retired or quarantined.

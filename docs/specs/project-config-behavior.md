# Debug80 Project Config Behavior (current design)
...
## Key files and paths
- Project config: `.vscode/debug80.json` (default scaffold), or `debug80.json` at the workspace root, or `.debug80.json`.
- Launch config points to it: `${workspaceFolder}/.vscode/debug80.json` (root paths still work if explicitly set).
...
## Config discovery
- Inputs: explicit `projectConfig` in launch, then `.vscode/debug80.json`, then `debug80.json`, then `.debug80.json`, then `package.json` containing `debug80`.
...
- Creates needed directories (including `.vscode` and target/output dirs).
...
- Use `.vscode/debug80.json` created by the scaffold command (root configs still work).
- Use the launch that references `${workspaceFolder}/.vscode/debug80.json`.

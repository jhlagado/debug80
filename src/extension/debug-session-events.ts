...
function handleAssemblyFailedEvent(
  ...
  { assemblyDiagnostics, platformViewProvider }: DebugSessionEventDependencies
): void {
  if (body === undefined) {
    return;
  }
  ...
  const summary =
    body.diagnostic?.message ?? body.error?.split(/\r?\n/, 1)[0] ?? 'Assembly failed';
...
const debugSessionCustomEventHandlers: Record<string, DebugSessionCustomEventHandler> = {
  ...
  'debug80/sessionStatus': (evt, deps) =>
    handleSessionStatusEvent(evt.body as { status?: string } | undefined, deps.platformViewProvider),
...
export function registerDebugSessionHandlers({
  ...
  context.subscriptions.push(
    vscode.debug.onDidTerminateDebugSession((session) => handleDebugSessionTerminated(session, deps))
  );

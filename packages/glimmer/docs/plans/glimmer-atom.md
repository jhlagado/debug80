...
Build the Atom version of Glimmer as a separate package in the Debug80
monorepo. Its transition name is `@jhlagado/glimmer-atom`. It keeps the `.glim`
language and the existing reactive runtime semantics, but it does not reuse the
AZM generator.

The current `@jhlagado/glimmer` package remains the working AZM implementation
and the migration oracle. Once the Atom implementation has reached byte and
behavioural parity on the supported language, it can replace the ordinary
`@jhlagado/glimmer` package. The temporary name is a migration boundary, not a
second language.
...
The reusable part of Glimmer ends at its checked program model. The existing
generator is deliberately AZM-shaped:
...
several instructions for one Glimmer body line, so the old label-and-text
...
profile model all describe Glimmer rather than AZM. They should be shared after
the new backend has exposed the right seam.
...
```text
packages/glimmer                 current AZM implementation and oracle
packages/glimmer-atom            new Atom implementation
packages/glimmer-atom/src
...
`@jhlagado/glimmer-core`. The two packages should then depend on that core,
...
buildGlimmerAtomProgram(entryPath, options) -> {
  diagnostics,
  warnings,
  artifacts,
  mappedSegments,
...
The CLI is initially `glimmer-atom`. It supports the same useful modes as the
current command:

```text
glimmer-atom generate entry.glim
glimmer-atom check entry.glim
glimmer-atom build entry.glim
glimmer-atom deps entry.glim
```
...
After migration, the package and command names become `@jhlagado/glimmer` and
`glimmer`. There is no new source extension. Glimmer source remains `.glim` and
all generated or imported assembly remains `.asm`.

## The lowering pipeline

```text
.glim entry and parts
        |
        v
parse, merge, validate, schedule
        |
        v
checked Glimmer program + source spans
        |
        +--> layout evaluator
...
program leaves its Glimmer body. The build API may pass the same bytes directly
...
provides the required semantics. Glimmer must not construct native Z80
descriptors or call internal sink hooks itself.

## Atom assembly dialect inside a Glimmer body

The lasting body contract is no longer “verbatim AZM.” It is:

> A block body is core Atom assembly plus Glimmer intrinsics. The host lowers
...
and it reports the original `.glim` position for every unsupported construct.
It performs these translations:

| Glimmer body form                                       | Atom output                                                   |
| ------------------------------------------------------- | ------------------------------------------------------------- |
...
position that is not a Z80 mnemonic, an Atom directive, or a declared Glimmer
intrinsic is an error. Glimmer must never pass an unknown pseudo-operation to
Atom and hope it is accepted.

Existing `.glim` programs use underscore locals and AZM expressions. The
migration tool accepts and lowers those known forms. New documentation teaches
dot-private labels and Atom expressions so new source does not depend on the
compatibility path.

## Lowering AZM-owned features

Atom remains a small assembler. The Glimmer host owns the higher concepts that
the Glimmer language exposes:
...
Glimmer source names remain descriptive and are not limited to eight
...
Glimmer `part` keeps its present meaning: it merges Glimmer declarations into
one checked program and namespace. It is not textual inclusion.

Glimmer `import` keeps its user-level meaning but changes its implementation:
the imported `.asm` file is an Atom source part. The build resolves paths
relative to the declaring `.glim` file, confines them to the project root,
...
- bytes that implement a user's body or declared resource map to `.glim`;
...
interface GeneratedLineOrigin {
  ...
  origin: { kind: 'glim'; file: string; line: number; column: number } | { kind: 'generated' };
...
lowering can therefore map one Glimmer line to one instruction, several
...
After Atom renders its native D8 map, Glimmer composes it as follows:

1. Validate the Atom map as D8 v1.
2. Find segments belonging to generated parts by generated line.
3. For a `glim` origin, move the segment to the `.glim` file and replace only
...
6. When one Glimmer line expands to several instructions, map every resulting
...
7. For an intrinsic expansion, add D8 macro metadata with the intrinsic name
   and the `.glim` call site. This is additive D8 v1 data.
8. Rebuild `fileList` deterministically with project-relative `/` paths.
9. Set the generator to `glimmer-atom`, record the `.glim` entry and generated
...
replaced in the public D8 symbol set by an alias carrying the original Glimmer
name, declaration line, value or address, and a stable Glimmer identity. Runtime
...
At minimum, D8 maps these Glimmer-authored bytes:
...
resource line is reported at the original `.glim` position. A failure in
generated glue points at the generated `.asm` file. Glimmer generation and
...
- bind breakpoints on a direct body instruction and on a multi-instruction
  intrinsic expansion;
- resolve every emitted address back to either the expected `.glim`, generated
...
1. Build the existing Glimmer program through the AZM implementation.
2. Build the same checked program through the Atom implementation.
...
the Atom implementation rejects it at its `.glim` or imported `.asm` position.
...
Debug80 initially gains a `glimmer-atom` backend ID. Project configuration
selects it explicitly because `.glim` alone cannot distinguish the two
implementations:

```json
{
  "sourceFile": "src/main.glim",
  "assembler": "glimmer-atom",
  "outputDir": "build",
  "artifactBase": "main"
}
```

The backend calls `buildGlimmerAtomProgram` in process and consumes its HEX and
D8 artifacts just as the present Glimmer backend does. There is no subprocess,
listing parser, or extension-based guess between AZM Glimmer and Atom Glimmer.

When Atom becomes the sole Glimmer implementation, `assembler: "glimmer"` and
`.glim` inference select it. Existing projects that must retain the old compiler
can temporarily use an explicit `glimmer-azm` compatibility backend.
...
- Create `packages/glimmer-atom` with library and CLI entry points.
- Copy the current parser, validation, model, and generic profile tests.
- Add source spans without changing accepted `.glim` programs.
...
### 5. Integrate Debug80 and migrate the name

- Add the explicit `glimmer-atom` backend and target configuration.
- Run the existing Dot, Tetro, and Sprite Chase headless scenarios through it.
- Publish the transition package only after its packed API works outside the
  monorepo.
- Make Atom Glimmer the ordinary backend after the complete acceptance corpus
  is green.
- Archive the AZM generator after an agreed compatibility period.

## Completion gate

The Atom implementation is ready to replace the current Glimmer package when:

- every shipped `.glim` example either builds byte-identically to the AZM
  oracle or has an approved, measured semantic difference;
...
This work does not add macros to Atom, teach the resident assembler about
Glimmer, move the Node compiler into Z80 memory, or redesign Glimmer's reactive
semantics. It does not use a filename extension to select an assembler. The
later TEC-1 shell can consume the same prepared source-part and streaming Atom
interfaces, but it is a separate measured project after the Node implementation
is complete.

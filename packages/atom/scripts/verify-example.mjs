...
try {
  ...
  assert.deepEqual(await fs.readFile(path.join(build, "main.bin")), expected);
  assert.equal(await fs.readFile(path.join(build, "main.hex"), "utf8"), [
  ...
  const listing = await fs.readFile(path.join(build, "main.lst"), "utf8");
  ...
  const d8 = JSON.parse(await fs.readFile(path.join(build, "main.d8.json"), "utf8"));

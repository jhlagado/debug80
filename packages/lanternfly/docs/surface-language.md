# Lanternfly surface language draft

Status: current design record
Edition: discussion draft 0.3
Implementation status: no compiler exists

This document records the surface-language decisions made after the 0.2
[working specification](specification.md). It is the current authority when its
syntax or routine model conflicts with that older specification. Numeric
semantics, address classes, backend contracts and hosted-body rules remain in
the working specification until the documents are reconciled into one edition.
...
A scalar constant occupies no writable storage. Static immutable tables may
eventually require either aggregate `const` storage or a separate data
declaration; that spelling remains open.
...
The initial arithmetic operators are `+`, `-`, `*`, `/`, `^` and `mod`.
...
Exact power and shift precedence remains subject to parser examples.
...
- constant aggregate storage and table syntax;
- exact shift and power precedence;

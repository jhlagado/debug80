...
Nine constraints broke the first draft of that source. Seven are language
...
| --- | --- |
| `u8`, `u16`, `boolean` | yes |
| `i16` | not yet — the parser's displacement arithmetic will |
...
| recursion | not in the tokenizer; the parser needs it |
| `if` / `else` | yes |
| `while` | yes |
| `for … until` | yes |
| `for … to` | no |
...
| `readSourceByte` | yes |
| the other four intrinsics | not in the tokenizer |
...
Source: 611 lines, of which the character-class table is 65 lines.
Excluding that table the tokenizer proper is 546 lines.

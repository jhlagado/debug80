...
/**
 * The three-pass spike, against the exit criteria in
 * `docs/three-pass-spike.md`.
 *
 * The claim under test is that the layout pass and the emission pass agree on
 * every address, which four planning documents assert and none had executed.
 */
...
describe("the address check fires when a lowering is broken", () => {
  const source = "var x as u8\nx = 3\nwhile x <> 0\nwriteCodeByte(x)\nx = x + 255\nend\n";

  it("compiles clean without sabotage", () => {
    expect(() => compile(source)).not.toThrow();
  });

  it("catches emission producing an extra byte", () => {
    expect(() => compile(source, { sabotage: "extra-byte" })).toThrow(
      /layout produced \d+ bytes and emission produced \d+/,
    );
  });

  it("catches a jump emitted shorter than it was laid out", () => {
    expect(() => compile(source, { sabotage: "short-jump" })).toThrow(MicroError);
  });

  it("catches a label that moved between the passes", () => {
    expect(() => compile(source, { sabotage: "moved-label" })).toThrow(
      /layout said \d+, emission reached \d+/,
    );
  });
});
...
describe("compilation is deterministic", () => {
  it("the same source twice gives identical bytes", () => {
    ...
    expect(second.layoutLength).toBe(first.layoutLength);
  });
});
  ...
  it("reports the layout of a representative program", () => {
    ...
    console.log(
      [
        ...
        `  label addresses ${result.labels.map((a) => `0x${a.toString(16)}`).join(", ")}`,
      ].join("\n"),
    );

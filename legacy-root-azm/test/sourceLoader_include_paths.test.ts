...
import { resolveIncludeCandidates } from '../src/sourceIncludePaths.js';
...
describe('sourceIncludePaths', () => {
  it('orders relative-to-importer first, then each include dir in order', () => {
    ...
    const got = resolveIncludeCandidates(fromSource, spec, includeDirs);
  ...
  it('skips include-dir steps that duplicate an earlier candidate', () => {
    ...
    const first = normalizePath(resolve(dirname(fromSource), spec));
